...
逐渐的，标准库中未包含的其他包逐一出现。这些类也就是我们所说的 __扩展（extensions）__，这些扩展又有两种： _标准（standard）_ 与 _非标准（not standard）_ 扩展。标准扩展时那些 Sun 公司认为是官方的扩展，他们与实验性扩展、早期版本或者说 beta 版，这样的或许永远也不会投入使用的包，有所不同。
